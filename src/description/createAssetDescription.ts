...
export function contentValidationError(content) {
    ...
    for (let i = 0; i < contentLength; i++) {
        ...
        if (!content[i].hash.startsWith('Qm')) return `element ${i} of "contents" should be a valid CIDv0 hash (received ${content[i].hash})`
    }
}
export function mainValidationError(opts: AssetDescription) {
    ...
    for (let i = 0; i < mainLength; i++) {
        ...
        if (!entry.entryPoint) return `all elements of "main" should have an "entryPoint" (received ${JSON.stringify(entry)})`
        if (!entry.bodyType) return `all elements of "main" should have a "bodyType" (received ${JSON.stringify(entry)})`
        if (!opts.contents.filter(_ => _.name === entry.entryPoint).length) return `entrypoint ${entry.entryPoint} was not present in the contents of ${opts.id}`
    }
}
...
export function i18nValidationError(i18n: { code: string, text: string }[]) {
    ...
    for (let i = 0; i < i18nLength; i++) {
        ...
        if (!entry.code) return `all elements of "i18n" should have an "code" specifying which language is the entry translatng to (received ${JSON.stringify(entry)})`
        if (!entry.text) return `all elements of "i18n" should have a "text" value with the corresponding translation (received ${JSON.stringify(entry)})`
...
export function validate(opts: AssetDescription) {
    return idValidationError(opts.id)
